...
| 🧵 **会话管理** | 历史保存/改名/删除、游标分页加载与全文搜索定位;消息支持编辑、复制、收藏、重发、删除,可导出 HTML 邮件 |
...
| 🪞 **深度换肤** | 12 套聊天皮肤、60 种页面背景、面板与代码配色,深浅色一键切换 |
...
| 🧠 **会话记忆** | 关键内容存档 + 历史检索,AI 拥有角色级长期记忆,跨会话不重复产出 |
| **文档生成** | |
| 📝 **Word 文档** | 富文本内容块(标题/段落/列表/表格/引用/目录/分页)+ 字体样式与页面方向,一键生成可下载链接 |
| 📊 **Excel 文档** | 多工作表、单元格类型与格式、合并单元格、列宽/批注/下拉验证/冻结首行 |
| 📄 **PDF 文档** | Markdown 正文渲染为版式规整的 PDF,支持标题/列表/表格/代码块/引用,方向/字号可选 |
| **扩展** | |
...
| 类型 | 数量 | 内容 |
|------|------|------|
| 聊天皮肤 | 12 套 | 复古 Win95、明亮现代、暗色现代、纸墨阅读、翡翠绿、终端黑、珊瑚晚霞、国风水墨、云海仙门、天空、深海、像素 |
...
- **个性化到极致**:12 套复古/现代皮肤、60 种背景配色、音色克隆与设计的自由组合
...
| `ai_chat_favorite` | 会话数据 | 消息收藏快照(仅结构,不含真实数据) |
