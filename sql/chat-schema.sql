-- ============================================================
-- qingsong-chat 会话/消息表结构(仅 schema,不导真实数据)
-- 来源:qingsong-backend/src/main/resources/db/chat-schema.sql
-- 说明:以下表存储用户真实对话内容,开源仓库【只保留建表语句】,
--       任何情况下不得提交真实数据行。
-- ============================================================
...
(
    ...
    session_no      VARCHAR(64)  NOT NULL COMMENT '会话业务ID',
    biz_type        VARCHAR(32)  NOT NULL DEFAULT 'chat' COMMENT '业务类型',
    ...
    KEY idx_biz_role_lastmsg (biz_type, role_code, last_message_at),
    KEY idx_deleted_lastmsg (deleted, last_message_at)
) COMMENT = 'AI会话表';

CREATE TABLE IF NOT EXISTS ai_chat_message
(
    id              BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    message_no      VARCHAR(64)  NOT NULL COMMENT '消息业务ID',
    ...
    content_format  VARCHAR(16)  NOT NULL DEFAULT 'TEXT' COMMENT '内容格式',
